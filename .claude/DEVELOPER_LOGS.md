...
Stage 1 stops short of the goal, though, and that matters. It enforces sign-in and those admin-only actions, and re-checks access on every request rather than caching it in a cookie, so being removed from your last artist stops you on your next click. But nothing yet filters profiles, the digest or verdicts by artist: every signed-in member can still see and edit every artist's work, not just their own. That filtering is stage 2. Until stage 2 ships, only admins should be given access on the People page.
...
The safety net is `tests/test_web_access.py`. Signed in as someone on a different artist, it calls every registered route with real ids from the other artist's data and expects 404, or 403 for admin-only actions. It fails if a route is added without being listed. Its sibling `test_web_access_sweeps.py` walks everything again as someone who was just taken off their only artist, and once more with every id too big for Postgres, where the answer must be not found, never a crash. Reviews along the way found write routes that could answer 404 *after* they had already committed, so the walk doesn't trust status codes: it snapshots everything the other artist owns, plus the budget, the People lists and the run queue, and compares them afterwards. To prove it wasn't just passing, we deleted one `require_outreach` line and watched it fail. Then we moved one profile check to after the commit, so the route still said 404, and watched the snapshot catch the change the status code hid. One surprise: FastAPI 0.141 nests included routers, so the plan's "list `app.routes`" found just two routes. The walk flattens them with `iter_route_contexts` instead.
