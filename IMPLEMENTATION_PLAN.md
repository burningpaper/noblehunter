...
Status: Shipped 2026-09-17. Migration 0007 applied to Neon, `db grant` re-run, `main` deployed to Vercel, the runner restarted on the new code, and a real pitch sent to a real curator.
...
- ✅ 1829 passed, 6 skipped. Lint clean.
- ✅ **Google consent — settled 2026-09-17, and it went our way.** Connecting the first real mailbox returned both scopes: the callback URL carried `gmail.readonly` *and* `gmail.send`. The unverified-app warning does appear, and an owner can click through it — but that screen is why nobody outside Jarred's own two accounts should be invited until the app passes Google verification, which for a restricted scope can take weeks.
- ✅ Shipped: Vercel environment variables, Neon migrated to 0007, `db grant` re-run, pushed, runner restarted, one real pitch sent. Two things the checklist didn't predict, each costing an hour: the Gmail API had never been enabled in the Google Cloud project, and `/mail/callback` had never been registered as a redirect URI — sign-in's callback is a different URL.
- ✅ Found by using it, and fixed: pitches signed with an email local part rather than a name; a scraped address emailed exactly as scraped; a 403 that told people to reconnect a working mailbox when the real cause was a disabled API; a LaTeX artifact in a subject line, and the guard for it standing one step upstream of the send path; message times rendered in UTC as though local; no favicon at all, which a new member reported as a broken login; the digest mixing two artists' entries; and brief writing spending past the nightly cap.
- ⏳ Known gaps, none breaking today: the two conversation settings can't be set from YAML, because `core/profile_import.py` writes settings onto the model without the validator; a digest that runs twice in one day counts the morning's emailed entries in both `taken` and `open_now`, so the evening top-up is over-conservative; "the night in numbers" ignores the profile filter; and **Ask Claude and the pitch writer spend on the shared Anthropic key with no cap and no ledger** — harmless at one user, now shared by three sign-ins, and the next real gap to close.
