...
One more change is structural rather than a feature: pytest now turns SQLAlchemy's "cartesian product" warning into a test failure. A query across artists, profiles and members that forgets a join doesn't just run slow, it can return every artist's rows instead of one, so that warning is exactly the shape of bug this stage exists to prevent. Better to fail the test than leak the rows.
