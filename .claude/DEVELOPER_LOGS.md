...
## 2026-09-18 (later): The nine cents that were never about nine cents

Run 8 spent $2.0853 against a $2.00 cap. Nine cents over. Nobody would notice that on a statement, and that is precisely why it was worth chasing — the overspend wasn't the bug, the mechanism was. Three stages of the nightly run call Claude. Two of them ask what's left before they spend. The third never asked.

Contact research works out `nightly_claude_budget(session) - run.llm_spend_usd`, hands it down as `spend_cap_usd`, and checks it before every single lead. Fit judging carries its own `budget` and stops the moment `self.spend >= self.budget`. Brief writing did neither: `digest_stage` called `build_digest` with no budget argument at all, and `_write_brief` called Claude for every entry and added the cost *afterwards* — `summary.spend_usd += brief.spend_usd`, a running total that nothing ever consulted. It spent whatever the night had already spent, plus one brief per entry, and then reported the damage as a fait accompli. Sixteen entries, sixteen briefs, nine cents over. The two profiles now carry digest targets of 10 and 20, so the near-term shape of this is thirty briefs a night stacked on an already-spent budget. Nine cents is the version of this bug we caught early.

**The obvious fix is the wrong one, and that is the whole story.** Cap the spend, stop writing briefs, stop the loop — and one heavy night the person opens the digest to eleven leads instead of sixteen, because the money ran out at eleven. That trade is upside down. The expensive, scarce, genuinely hard-won thing in this pipeline is a *lead*: a curator who is alive, on-genre, reachable, and not pitched in the last ninety days. Everything upstream — search, fetch, fit, contact research — exists to produce one. The brief is a paragraph of prose written from facts the database already holds. Trading a lead for a cent's worth of prose would have been a far worse bug than the one being fixed, and worse still, an invisible one: a short digest reads like a quiet night, not like a budget event. Nobody would ever have filed it.

So briefs degrade rather than truncate. When the cap is reached, `_write_brief` returns `template_brief(request)` — the same plain brief that has always covered a flaky API — and the entry is written exactly as it would have been. Every lead the run found still reaches the morning. The digest gets plainer, never shorter. That fallback path was already well trodden; this change only gives it a second reason to fire, which is why the diff is as small as it is.

The boundary was a real decision rather than a coin toss. Research treats a lead as affordable while `spend < cap`, so it stops at `spend >= cap`; fit judging skips while `self.spend >= self.budget`. Two stages already agreed, so the digest uses `>=` as well. That matters more than it sounds: a budget of exactly $0.00 has to mean *every* brief is plain, and under `>` a zero budget would quietly buy one brief before noticing — as would a night that lands exactly on the cap. Three stages disagreeing about what "budget reached" means is how you get the bug nobody can reproduce.

The cap is read once, at stage start, the way research reads it. Entries are written in a loop, and a per-entry re-read would be both slower and harder to reason about — worse, it would make the digest's behaviour depend on when in the loop somebody edited the budget in the web app.

Two counters, not one, and this is the part a future debugger will thank us for. `template_briefs` already existed and already meant "plain brief", but it covers briefs that fell back because Claude *errored*. A budget fallback and an API failure produce identical-looking output and demand completely different responses — raise the cap, versus go and find out what Claude is doing. So `budget_briefs` sits beside it, and the run report says which happened: the existing "because Claude couldn't write them" line is untouched, and a new one follows the research stage's phrasing, adapted honestly — "3 plain briefs, because the nightly Claude budget was reached. Every lead is still here; raise it on the Profiles page for written briefs." The second sentence is doing real work. Without it the reader's first thought on a plainer morning is "what did I lose?", and the answer — nothing — is the one thing the line must say out loud.

Eight tests, written first and watched failing, each for its own reason: `TypeError` on the `spend_cap_usd` keyword that didn't exist, a separate `TypeError` on `DigestSummary(budget_briefs=...)`, `KeyError` where the stage wasn't passing a cap at all. The four that matter most are the budget ones in `tests/test_digest.py`: plenty of money means Claude writes every brief as before; a budget already spent means *no Claude call is made and all three leads still become entries* with template briefs; a budget that runs out partway gives the earlier entries Claude briefs and the later ones plain ones, with the entry count unchanged either way. That last assertion — `len(entries) == 3` in every one of those tests — is the one guarding the thing this change is actually about.
